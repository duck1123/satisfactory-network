(ns satisfactory.components
  (:require

   ;; [satisfactory.events.accounts :as e.accounts]
   [satisfactory.events.components :as e.components]
   ;; [satisfactory.events.users :as e.users]
   ;; [satisfactory.translations :refer [tr]]
...
;;                         :on-change #(rf/dispatch [change-handler (target-value %)])}]
;;               (concat [[:option {:value ""} ""]]
;;                       (for [{:keys [db/id satisfactory.spec.accounts/name]} items]
...
;;                      (for [{:keys [db/id satisfactory.spec.currencies/name]} currencies]
;;                        ^{:key id} [:option {:value id} name])))]]]))
...
;;                   :on-change #(rf/dispatch [change-handler (target-value %)])}]
;;         (concat [[:option {:value ""} ""]]
;;                 (for [{:keys [db/id satisfactory.spec.users/name]} items]
;;                   ^{:key id}
